...
const vec2 offsets[] = vec2[](vec2(-1.0, -1.0), vec2(-1.0, 1.0), vec2(1.0, -1.0), vec2(1.0, 1.0));

void main() {
    vec4 pos_view = cam.mat_view * vec4(in_pos.xyz, 1.0);
    out_offset = offsets[gl_VertexIndex];
    pos_view.xy += out_offset;
    gl_Position = cam.mat_proj * pos_view;
    out_color_tint.xyz = in_vel.xyz;
    out_color_tint.w = smoothstep(0.0, 1.0, in_pos.w);
}
